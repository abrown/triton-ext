...
def run(search_dir: str, criteria: str) -> str:
    ...
    if not candidates:
        raise FileNotFoundError(
            f"No artifact directory found matching criteria: {criteria}. Try downloading the artifact from GitHub (if \
                cached): `python ci/download-artifact.py {criteria}`.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python pick-local-artifact.py <project>[-<commit>[-<os>[-<arch>]]]"
        )
        sys.exit(1)
