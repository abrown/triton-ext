#!/usr/bin/env python3
"""
Retrieve a pre-built LLVM artifact from the Triton project's Azure blob storage and expand it in
the current directory.

Triton's CI builds LLVM and uploads the resulting tarballs to a public Azure blob container at:
    https://oaitriton.blob.core.windows.net/public/llvm-builds/

Artifact names follow the pattern:
    llvm-<short-commit>-<os>-<arch>-<build-number>

where <os> is one of: ubuntu, almalinux, macos, windows.

This script accepts a single, hyphen-delimited argument that can be incrementally more specific
(e.g., `llvm`, `llvm-<commit>`, `llvm-<commit>-<os>`, etc.):

- `llvm`: the project (required; currently only `llvm` is hosted on Azure)
- `<commit>`: a short (8-char) LLVM commit hash (optional; defaults to the hash pinned in Triton)
- `<os>`: the target OS (optional; defaults to current system; one of: ubuntu, almalinux, macos, windows)
- `<arch>`: the architecture (optional; defaults to current system; one of: x64, arm64)
- `<build-number>`: the build number (optional; defaults to the number pinned in Triton)
...
def fetch_llvm_build_info(triton_rev):
    ...
    logging.debug(f"Fetching LLVM build info from: {url}")
...
def download_artifact(artifact_name):
    ...
    if os.path.exists(artifact_file):
        logging.debug(f"Artifact already exists locally: {artifact_file}")
        return artifact_file

    url = f"{AZURE_BASE_URL}/{artifact_file}"
    logging.debug(f"Downloading artifact from: {url}")

    response = requests.get(url, stream=True)
    if response.status_code == 404:
        logging.error(f"Artifact not found: {url}")
        logging.error(
            "Check that the commit, OS, arch, and build number are valid.")
        sys.exit(1)
    response.raise_for_status()
...
def verify_checksum(artifact_file, expected_sha256):
    ...
    logging.debug(f"Verifying checksum: {artifact_file}")
    ...
    if actual != expected_sha256:
        logging.error(f"Checksum mismatch for {artifact_file}:")
        logging.error(f"  expected: {expected_sha256}")
        logging.error(f"  actual:   {actual}")
        os.remove(artifact_file)
        sys.exit(1)
    logging.debug(f"Checksum verified: {actual}")
...
def main(project: str, commit: str | None, os_name: str | None,
    ...
    if not commit or not build_number:
        ...
        logging.debug(f"Fetched LLVM build info: {build_info}")
    ...
    if not build_number:
        ...
        logging.debug(f"Found LLVM build number: {build_number}")

    # If no OS or architecture is provided, probe the current system.
    probed_os, probed_arch = probe_sysinfo()
    logging.debug(f"Probed system: {probed_os}-{probed_arch}")
    if not os_name:
        os_name = probed_os
    if not arch:
        arch = probed_arch
...
if __name__ == "__main__":
    ...
    if project != "llvm":
        logging.error("Invalid project name; expected 'llvm'.")
        logging.error(f"Usage: {USAGE}")
        sys.exit(1)
